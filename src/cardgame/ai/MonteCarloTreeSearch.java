...
public class MonteCarloTreeSearch {
	 ...
	 * Spieler anzugreifen, ohne zu toeten, ist theoretisch betrachtet nicht immer die beste Wahl,
	 * da durch den Angriff, die Evo-Schilder veraendert werden, wodurch der weitere Spielverlauf 
	 * entscheidend, nachteilig, beeinflusst werden kann.
	 * Bedingungen:
	 * - Enemy hat keine Karten mehr auf Battleground.
	 * - Enemy hat weniger Schilde, als self Karten, die noch nicht angegriffen haben.
     ...
     * Karte legen: ng[0..4] (new gamecard).
     * Specialcard legen: ns[0..4][0..7]+ (new specialcard)(0..3: self, 4..7: enemy) .
     * Spieler angreifen: g[0..3]-1 (-1 for the player)
     * Angreifen: g[0..3][0..3]
     * @param n Node n(alte Node)
     * @return Node (neue, aus alter resultierende Node)
     * @throws LogicException 
     */
    private Node makeTransition(Node n) throws LogicException{
    	Node newNode = null;
    	 ...
    	 * Bedingung ng:
    	 * - Platz auf eigenem Battleground.
    	 * - Man hat auf der Hand eine GameCard/Specialcard.
    	 */
    	...
    	while(cardsOnHand.size() > 0){
	    	...
	    	}else if(card instanceof GameCard && g.getMyField(self_id).getCountBattlegroundMonster() < Playground.ROW){
    	...
    	/**Transition: Mit GameCard angreifen g[0..3][0..3] oder Spieler angreifen g[0..3]-1:
    	 * Bedingungen(g[0..3][0..3]):
    	 * - Enemy hat Karten.
    	 * - Self hat Karten, die noch nicht angegriffen haben.
    	 * Bedingungen(g[0..3]-1):
    	 * - Enemy hat keine Karten mehr auf Battleground.
    	 * - Self hat Karten, die noch nicht angegriffen haben. 
 		 * UnterBedingung: Spieler toeten
 		 * - Enemy hat keine Karten mehr auf Battleground. 
 		 * - Enemy hat weniger Schilder, als Self noch Karten zum Angreifen.
    	 */
    	//Karten sammlen:
    	...
    	while(cards.size() > 0){
    		GameCard current = cards.get(random.nextInt(cards.size()));
